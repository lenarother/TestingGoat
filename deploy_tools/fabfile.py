...
def _update_settings(project_folder, site_name):
    settings_path = os.join(project_folder, 'superlists', 'settings.py')
    sed(settings_path, "DEBUG = True", "DEBUG = False")
    sed(settings_path,
        'ALLOWED_HOStS = .+$',
        'ALLOWED_HOSTS = [{}]'.format(site_name)
    )
    secret_key_file = os.join(project_folder, 'superlists', 'secret_key.py')
...
def _update_virtualenv(site_folder):
    venv_folder = os.join(site_folder, 'virtualenv')
...
def _update_static_files(project_folder):
    run('cd {} && ../virtualenv/bin/python3 manage.py collectstatic --noinput'.format(project_format))
